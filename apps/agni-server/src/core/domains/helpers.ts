...
export const isValidColor = (color: string): boolean => {
    color = color.trim(); // Supprime les espaces inutiles

    // HEX
    if (/^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(color)) return true;

    // RGB
    if (/^rgb\(\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*,\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*,\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*\)$/i.test(color)) 
        return true;

    // RGBA
    if (/^rgba\(\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*,\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*,\s*(?:25[0-5]|2[0-4][0-9]|1?[0-9]{1,2})\s*,\s*(?:0?\.\d+|1(?:\.0+)?)\s*\)$/i.test(color)) 
        return true;

    // HSL
    if (/^hsl\(\s*(?:360|3[0-5][0-9]|[12]?[0-9]{1,2})\s*,\s*(?:100|[1-9]?[0-9])%\s*,\s*(?:100|[1-9]?[0-9])%\s*\)$/i.test(color)) 
        return true;
