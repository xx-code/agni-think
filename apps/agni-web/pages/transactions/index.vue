...
import type { EditTransactionType, TransactionTableType, RecordTableType, TransactionType } from "~/types/ui/transaction";
import type { FilterTransactionQuery } from "~/types/api/transaction";
import { ModalEditTransaction } from "#components";
import { fetchTransaction } from "~/composables/invoices/useTransaction";
import useUpdateTransaction from "~/composables/invoices/useUpdateTransaction";
import useCreateTransaction from "~/composables/invoices/useCreateTransaction";
import type { FormFilterTransaction } from "~/types/ui/component";
import useCompleteTransaction from "~/composables/invoices/useCompleteTransaction";
...
const tableColumn: TableColumn<TransactionTableType>[] = [
    ...
    },
    {
        ...
        header: 'Description',
        cell: ({ row }) => {
            const recordCount = row.original.records.length
    ...
    const options = [
        {
            ...
            onSelect: () => openTransaction(rows.original.id)
        },
        {
            label: 'Supprimer',
                    ...
                    <div class="px-4 py-4 bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <div class="space-y-3">
                            <h4 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                Articles ({{ row.original.records.length }})
                            </h4>
                            
                            <div 
                                v-for="record in row.original.records" 
                                :key="record.id"
