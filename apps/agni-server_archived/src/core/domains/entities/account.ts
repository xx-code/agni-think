...
export class Account extends Entity {
    ...
    private type: TrackableProperty<Account>
    private detail: TrackableProperty<IAccountDetail|undefined> 
    private currencyId: TrackableProperty<string>

    constructor(id: string, title: string, type: Account, currencyId: string, detail?: IAccountDetail, balance: number = 0) {
        super(id)
        this.title = new TrackableProperty<string>(title, this.markHasChange.bind(this))
        this.type = new TrackableProperty<Account>(type, this.markHasChange.bind(this))
        ...
        if ((type == Account.BROKING || type == Account.CREDIT_CARD) && detail === undefined) 
